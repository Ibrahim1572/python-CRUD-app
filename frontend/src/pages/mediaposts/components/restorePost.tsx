...
export default function RestorePost(){
    ...
    const deletePostFunc = async() =>{
        const response = await axios.patch('/api/deleteRestorePost?isDeleted=True', {'postData':{'postTitle':postTitle}})
    } 
    ...
    return (
        <div className="w-full max-w-[560px] flex flex-col gap-y-12 font-sans">
            ...
            <div className="flex flex-col items-center justify-center gap-6 bg-[#202020] border-b border-[#202020] p-10 rounded-none">
                {(postFound)?
                    (<>
            ...
            {(postFound)?(
                    <button
                        ...
                        className="bg-transparent text-white border border-white/50 rounded-none py-4 text-[14.4px] font-light uppercase tracking-[0.2px] cursor-pointer transition-colors duration-150 hover:bg-[#1EAEDB] hover:opacity-70"
                    >
                        Restore
                    </button>
                ):(<></>)}
